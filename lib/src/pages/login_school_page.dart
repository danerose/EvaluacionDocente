...
class LoginSchoolPage extends StatefulWidget{
  final user;
  LoginSchoolPage(this.user);
  @override
  State createState() => LoginSchoolState();
}

class LoginSchoolState extends State<LoginSchoolPage>{
final _formKey = GlobalKey<FormState>();
  @override
  Widget build(BuildContext context) {
    String user = widget.user.toString();
    if(user == '2'){user = 'Docente';}
    else if(user == '3'){user = 'Coordinador';}
    else{user = 'Administrador';}
    return Scaffold(
      body: _body(_formKey,user),
    );
  }
}
...
_flexibleSpaceBar(size,user){
  return FlexibleSpaceBar(
    title: TextContent('Login '+user,size,TextAlign.end,Colors.white,true),
